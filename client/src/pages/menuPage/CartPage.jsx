...
const CartPage = () => {
   ...
   const handleDelete =   (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://three-sisters-cafe-1.onrender.com/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
  ...
  const handleDecrease = (item) => {
    if (item.quantity > 1) {
      fetch(`https://three-sisters-cafe-1.onrender.com/carts/${item._id}`, {
        method: "PUT",
        headers: {
              ...
              <tbody>
                {cart.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                    ...
                    <td className="py-2 text-center text-slate-600">RS.{calculatePrice(item).toFixed(2)}</td>
